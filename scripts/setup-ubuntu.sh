#!/usr/bin/env bash
# =============================================================================
# gdrive-music — Setup Wizard (Ubuntu-based)
# Distros: Ubuntu 20.04+ · Linux Mint 20+ · Pop!_OS · Zorin OS · elementary OS
...
header() {
  ...
  echo "  ║    Ubuntu · Mint · Pop!_OS · Zorin · elementary         ║"
  echo "  ╚══════════════════════════════════════════════════════════╝"
  echo -e "${RESET}"
}
...
fail() {
  ...
  echo -e "  ${RED}╚══════════════════════════════════════════════════════════╝${RESET}"
  echo ""
  echo -e "  ${DIM}Fix the issue above and re-run: bash scripts/setup-ubuntu.sh${RESET}"
  echo ""
  exit 1
}
...
detect_audio() {
  if pactl info 2>/dev/null | grep -qi "pipewire"; then
    ...
    # Ubuntu 20.04 defaults to PulseAudio; 22.04+ ships PipeWire optionally
    echo "pulse"
  fi
}

install_cava_from_source() {
  info "cava not in apt repos — building from source..."
  echo ""
  ...
  if ! git clone --depth 1 https://github.com/karlstav/cava "$CAVA_TMP/cava" 2>&1 | tail -3; then
    ...
    fail "cava build" "git clone failed — check internet connection" \
      "Try: sudo apt install cava (Ubuntu 24.04+)"
  fi
  ...
  if ! cmd_exists cava; then
    fail "cava build" "cava compiled but binary not found in PATH" \
      "Check /usr/local/bin/cava exists"
  fi
...
echo -e "  ${DIM}Note:${RESET}     cava may be compiled from source on older Ubuntu"
echo ""
divider
echo ""
...
# Validate this is apt-based
if ! cmd_exists apt-get; then
  fail "System check" \
    "apt-get not found — this script is for Ubuntu-based distros only" \
...
# Detect Ubuntu base version (works for Mint/Pop too via UBUNTU_CODENAME or VERSION_ID)
UBUNTU_VER=$(grep "^UBUNTU_CODENAME\|^VERSION_ID" /etc/os-release 2>/dev/null \
  | head -1 | cut -d= -f2 | tr -d '"' | cut -d. -f1 || echo "0")
# Fallback: lsb_release
if [[ "$UBUNTU_VER" == "0" ]] && cmd_exists lsb_release; then
...
info "Base ver:  Ubuntu $UBUNTU_VER"
info "Pkg mgr:   apt"
...
# ── fuse3 vs fuse ────────────────────────────────────────────────────────────
# Ubuntu 20.04 ships fuse (v2); fuse3 is backported but may not exist.
# 22.04+ has fuse3 in main repos.
FUSE_PKG="fuse3"
if [[ "${UBUNTU_VER:-0}" -lt 22 ]]; then
  FUSE_PKG="fuse"
  info "Ubuntu < 22.04 detected — using fuse (v2) instead of fuse3"
fi
...
# ── rclone: official installer (apt version is often 1.x behind) ─────────────
info "Installing rclone via official installer (newer than apt version)..."
...
# ── cava: try apt first (works on Ubuntu 24.04+), build from source otherwise ─
info "Checking cava availability..."
if sudo apt-get install -y cava 2>&1 | grep -q "Unable to locate package\|has no installation candidate"; then
  install_cava_from_source
else
  success "cava installed via apt"
fi
...
if [[ -z "$FUSERMOUNT_BIN" ]]; then
  fail "fusermount" "fusermount not found — fuse install may have failed" \
...
if ! tar -xzf "$RMPC_TARBALL" -C "$RMPC_TMP"; then
  rm -rf "$RMPC_TMP"
  fail "rmpc extract" "tar extraction failed" "Re-run: bash scripts/setup-ubuntu.sh"
fi
...
[Service]
# Type=simple: rclone is not compiled with sd_notify support on Ubuntu.
# Type=notify causes systemd to kill the process before mount stabilises.
Type=simple
...
# Ubuntu/Mint/Pop!_OS install MPD as a system service that starts on boot.
# This conflicts with the per-user service. Disable system MPD first.
info "Disabling system MPD service (Ubuntu conflict fix)..."
